...
public class ComplianceChecker {
    ...
    public void checkMscGroups() {
        ...
        if (!mailsortProduct.equals(Product.UNSORTED)) {
            ...
            // Group by MSC + Lang
            Collection<List<Customer>> mscGroups = customers.stream()
                                                            .filter(item -> batchTypesToUkm.contains(item.getBatchType()))
                                                            .collect(Collectors.groupingBy(c -> c.getLang().name() + c.getBatchName() 
                                                                        + c.getSubBatch() + c.getMsc())).values();
            for (List<Customer> group : mscGroups) {

                int occurrences = group.size();

                if (occurrences < ukmMinimumTrayVolume) {
                    Customer c = group.get(0);
                    String msc = c.getLang().name() + c.getBatchName() + c.getSubBatch() + c.getMsc();
            ...
            if (mscsToAdjust.size() > 0) {
                ...
                for (Customer cus : customers) {
                    // Use full MSC including subbatch
                    String msc = cus.getLang().name() + cus.getBatchType().name() + cus.getSubBatch() + cus.getMsc();
                    // Switch to UNSORTED if in list
                    if (mscsToAdjust.contains(msc)) {
                        cus.updateBatchType(UNSORTED, presentationConfig.lookupRunOrder(UNSORTED));
                        // PB 19/02/19 : Switch Multis to UNSORTED singles when set in production config, otherwise leave as MULTIs
    ...
    public void calculateDPSCompliance() {
        if (!mailsortProduct.equals(Product.UNSORTED)) {
            ...
            LOGGER.info("Run total={}, total mailsort count={}, \n\tgood DPS count={}, bad DPS count={}, \n\tmaximum permitted default DPS={}, compliance level={} minimum compliance set to {}", customers.size(), totalMailsortCount, goodDpsCount, badDpsCount, maxBadDps, compliance, ukmMinimumCompliance);
    ...
    /**
     * Calculate how we are actually going to send this run, UNSORTED, SORTED via
     * MM, SORTED via OCR
     */
